...
@Component({
  ...
  templateUrl: './extra-modal.page.html',
  styleUrls: ['./extra-modal.page.scss'],
})
export class ExtraModalPage implements OnInit {
  @Input() feed: Feed;
  isUpdate = false;
  ...
  valueA: string;
  Tipo = "HoraExtra.";
  Titulo = "Jornada de Trabalho Superior a normal.";
  Descricao="jornada excedida, conforme marcações no relogio.";
  selected: any;
  KeyArray:any;
  KeyCpf:any;
  Token:string;
 ...
 if (this.isUpdate) {
    ...
    this.service.createPnt(pendencia).subscribe(
      ...
      const Mydata = {
        ...
        LargText: 'Hora Extra? dia: ' +  DateNN + '. Não se esqueça de justificar no aplicativo, para que tenhamos ciencia da permição e razão destas horas neste dia.',
        SImg: '',
        LImage: '',
      }
